...
def find_weak_pareto_improvment(allocation: list[list[float]],valuations: list[list[float]],cycle: list = None):
    ...
    #     candidates_for_e.append(valuations[agentA][currItem]*allocation[agentA][currItem]*rollingProd)

    #     currR = valuations[agentA][currItem]/valuations[agentB][currItem]
    #     rollingProd *= currR

    # e = min(candidates_for_e)
    e = 0.001
    ...
    while flag:
        ...
        for index in rangeList:
            ...
            currR = valuations[agentA][currItem]/valuations[agentB][currItem]
            logger.info("%s to %s item %d amount %f", agentA, agentB, currItem, e/rollingProd)
